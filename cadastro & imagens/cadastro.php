...
$insert_usuario = "INSERT INTO usuario (usuario_nome, usuario_email, usuario_senha) VALUES (?, ?, ?)";
$stmt = $connection->prepare($insert_usuario);

if ($stmt === false) {
    echo "Erro de preperação: " . $connection->error;
    exit();
}

$stmt->bind_param("sss", $usuario_nome, $usuario_email, $usuario_senha);


if ($stmt->execute()) {
    $ultimo_id = mysqli_insert_id($connection);
    $_SESSION["id"] = $ultimo_id;
    header("location: imagens_perfil.php");
} else {
